...
class TestIndexingPerformance:
    ...
    def test_type_dispatch_speedup(self):
        ...
        for i in range(100):
            ...
            # Structures
            clauses.append(
                Clause(Struct("dispatch", (Struct("s", (Int(i),)), Int(i * 30))), ())
            )
            # Lists (empty and non-empty)
            if i % 2 == 0:
                clauses.append(
                    Clause(Struct("dispatch", (Atom("[]"), Int(i * 40))), ())
                )
            else:
                clauses.append(
                    Clause(
                        Struct(
                            "dispatch", (Struct(".", (Int(i), Atom("[]"))), Int(i * 50))
                        ),
                        (),
                    )
                )
    ...
    def test_mixed_workload_performance(self):
        ...
        assert (
            avg_speedup >= 1.1
        ), f"Expected average speedup ≥1.1x, got {avg_speedup:.1f}x"
...
class TestMemoryOverhead:
    ...
    def test_memory_overhead_structural(self):
        ...
        if hasattr(engine_idx.program, "_index"):
            ...
            assert (
                total_entries <= 2 * num_clauses
            ), f"Index has {total_entries} entries for {num_clauses} clauses (>{2*num_clauses})"

            print(
                f"Index structural overhead: {total_entries} entries for {num_clauses} clauses"
            )
        else:
