...
class TestSemanticEquivalence:
    ...
    @given(
        num_clauses=st.integers(min_value=5, max_value=20),
        has_cut=st.booleans()
    )
    @settings(max_examples=30, deadline=None)
    ...
    def test_backtracking_points_identical(self, facts: List[int]):
        ...
        for val in facts:
            ...
            clauses.append(Clause(
                Struct("double", (Int(val), Int(val * 2))),
                ()
            ))
...
class TestIndexCorrectness:
    ...
    def test_no_non_matching_clauses_selected(self, clauses_data: List[Tuple[str, int]]):
    ...
    def test_order_preserved_within_selection(self, data):
        ...
        for i in range(num_per_type):
            # Structures
            clauses.append(Clause(Struct("ord", (Struct("s", (Int(i),)), Int(i * 30))), ()))
