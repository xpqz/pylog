...
class TestREPLSessionScenarios:
    ...
    def test_family_tree_session(self, mock_prompt_session, tmp_path):
        ...
        loaded_found = any(
            "loaded" in str(call).lower() or "success" in str(call).lower()
...
class TestREPLDisplay:
    ...
    def test_pretty_print_terms(self):
        ...
        # Test nested list
        nested = List(
            (List((Int(1), Int(2)), Atom("[]")), List((Int(3),), Atom("[]"))),
            Atom("[]"),
        )
    ...
    def test_format_multiple_solutions(self):
        ...
        assert (
            "3 solution" in formatted.lower()
            or "3 result" in formatted.lower()
            or formatted.count("X =") == 3
        )
