...
class TestLibraryPredicatesAcceptance:
    ...
    def test_append_acceptance(self):
        ...
        query1 = parser.parse_query("append([1,2], [3,4], X).")
        solutions1 = list(engine.solve(query1.goals))
        ...
        query2 = parser.parse_query("append(X, Y, [1,2,3]).")
        solutions2 = list(engine.solve(query2.goals))
        ...
        query3 = parser.parse_query("append([1,2], Y, [1,2,3,4]).")
        solutions3 = list(engine.solve(query3.goals))
    ...
    def test_member_acceptance(self):
        ...
        query1 = parser.parse_query("member(2, [1,2,3]).")
        solutions1 = list(engine.solve(query1.goals))
        assert len(solutions1) == 1  # Found
        
        query2 = parser.parse_query("member(4, [1,2,3]).")
        solutions2 = list(engine.solve(query2.goals))
        assert len(solutions2) == 0  # Not found
        
        # Mode 2: member(?Elem, +List) - generation
        query3 = parser.parse_query("member(X, [a,b,c]).")
        solutions3 = list(engine.solve(query3.goals))
    ...
    def test_reverse_acceptance(self):
        ...
        query1 = parser.parse_query("reverse([1,2,3], X).")
        solutions1 = list(engine.solve(query1.goals))
        assert len(solutions1) == 1
        ...
        query2 = parser.parse_query("reverse([], X).")
        solutions2 = list(engine.solve(query2.goals))
        assert len(solutions2) == 1
        assert solutions2[0]["X"] == Atom("[]")
        
        # Test checking mode
        query3 = parser.parse_query("reverse([a,b,c], [c,b,a]).")
        solutions3 = list(engine.solve(query3.goals))
        assert len(solutions3) == 1  # Correct
        
        query4 = parser.parse_query("reverse([a,b,c], [a,b,c]).")
        solutions4 = list(engine.solve(query4.goals))
    ...
    def test_between_acceptance(self):
        ...
        query1 = parser.parse_query("between(1, 3, X).")
        solutions1 = list(engine.solve(query1.goals))
        ...
        # Test checking mode
        query2 = parser.parse_query("between(1, 5, 3).")
        solutions2 = list(engine.solve(query2.goals))
        assert len(solutions2) == 1  # 3 is in range
        
        query3 = parser.parse_query("between(1, 5, 6).")
        solutions3 = list(engine.solve(query3.goals))
        assert len(solutions3) == 0  # 6 is out of range
        
        # Test edge case: low > high
        query4 = parser.parse_query("between(5, 1, X).")
        solutions4 = list(engine.solve(query4.goals))
...
class TestExceptionHandlingAcceptance:
    ...
    def test_catch_captures_thrown_terms(self):
        ...
        # Test 1: Exact match
        query1 = parser.parse_query("catch(throw(ball), ball, true).")
        solutions1 = list(engine.solve(query1.goals))
        assert len(solutions1) == 1  # Caught and recovered
        
        # Test 2: Variable catcher
        query2 = parser.parse_query("catch(throw(error(type, foo)), error(Type, Term), true).")
        solutions2 = list(engine.solve(query2.goals))
        assert len(solutions2) == 1
        ...
        query3 = parser.parse_query("catch(throw(ball), box, true).")
        solutions3 = list(engine.solve(query3.goals))
        assert len(solutions3) == 0  # Uncaught, fails
        
        # Test 4: Success passes through
        query4 = parser.parse_query("catch(true, _, fail).")
        solutions4 = list(engine.solve(query4.goals))
        assert len(solutions4) == 1  # Success, recovery not executed
        
        # Test 5: Failure passes through
        query5 = parser.parse_query("catch(fail, _, true).")
        solutions5 = list(engine.solve(query5.goals))
...
class TestOnceAcceptance:
    ...
    def test_once_no_choicepoint(self):
        ...
        query = parser.parse_query("test(X, Y).")
...
class TestParserAcceptance:
    ...
    def test_parser_accepts_valid_programs(self):
        ...
        query = parser.parse_query("grandparent(tom, Z).")
...
class TestPrettyPrinterAcceptance:
    ...
    def test_pretty_printer_handles_complex_terms(self):
        ...
        output = printer.pretty(complex_term)
...
class TestStage1Integration:
    ...
    def test_complete_program_execution(self):
        ...
        # Test fact queries
        query1 = parser.parse_query("person(alice).")
        assert len(list(engine.solve(query1.goals))) == 1
        
        # Test rule queries
        query2 = parser.parse_query("adult(X).")
        solutions2 = list(engine.solve(query2.goals))
        assert len(solutions2) == 2
        
        # Test list operations
        query3 = parser.parse_query("append([1, 2], [3, 4], X).")
        solutions3 = list(engine.solve(query3.goals))
        assert len(solutions3) == 1
        
        # Test builtins
        query4 = parser.parse_query("test_functor(foo).")
        assert len(list(engine.solve(query4.goals))) == 1
        
        query5 = parser.parse_query("test_univ.")
        assert len(list(engine.solve(query5.goals))) == 1
        
        query6 = parser.parse_query("test_arg.")
        assert len(list(engine.solve(query6.goals))) == 1
    ...
    def test_round_trip_property(self):
        ...
        for prog_text in programs:
            ...
            pretty_clauses = []
            for clause in program1.clauses:
                if clause.body:
                    head_str = printer.pretty(clause.head)
                    body_strs = [printer.pretty(goal) for goal in clause.body]
                    pretty_clauses.append(f"{head_str} :- {', '.join(body_strs)}.")
                else:
                    pretty_clauses.append(printer.pretty(clause.head) + ".")
            
            # Re-parse
            program2 = parser.parse_program("\n".join(pretty_clauses))
            
            # Should have same number of clauses
            assert len(program1.clauses) == len(program2.clauses)