...
class TestCutBarrierTracking:
    ...
    def test_cut_barrier_infrastructure_smoke_test(self):
        ...
        """
        prog = program(
            mk_rule("test", (Var(0, "X"),), Struct("choice", (Var(0, "X"),))),
    ...
    def test_barrier_restoration_across_clauses(self):
        ...
        """
        prog = program(
            mk_rule("test", (Var(0, "X"),), Struct("a", (Var(0, "X"),))),
            mk_rule("test", (Var(0, "X"),), Struct("b", (Var(0, "X"),))),
            mk_fact("a", Atom("1")),
            mk_fact("a", Atom("2")),
            mk_fact("b", Atom("3")),
            mk_fact("b", Atom("4")),
...
class TestCutBuiltin:
    ...
    def test_cut_removes_newer_choicepoints(self):
        ...
        prog = program(
            mk_rule(
                "test", (Var(0, "X"),), Struct("choice", (Var(0, "X"),)), Atom("!")
            ),  # Cut here
    ...
    def test_cut_preserves_older_choicepoints(self):
        ...
        prog = program(
            ...
            mk_rule(
                "inner", (Var(0, "Z"),), Struct("choice2", (Var(0, "Z"),)), Atom("!")
            ),  # Cut here
    ...
    def test_cut_as_last_goal(self):
        ...
        """
        prog = program(
            mk_rule("p", (Var(0, "X"),), Struct("choice", (Var(0, "X"),))),
            # Cut as last goal
            mk_fact("choice", Atom("a")),
            mk_fact("choice", Atom("b")),
    ...
    def test_green_cut_deterministic_behavior(self):
        ...
        """
        prog = program(
            mk_rule("det", (Atom("a"),), Atom("fact")),
            # Cut here would be green (no alternatives anyway)
            mk_rule("det", (Atom("b"),), Atom("fact")),
            mk_fact("fact"),
        )
        engine = Engine(prog)
    ...
    def test_red_cut_would_change_semantics(self):
        ...
        solutions = engine.run([Struct("max", (Atom("1"), Atom("2"), x_var))])
        ...
        list_term = Struct(
            ...
            (
                ...
                Struct(
                    ...
                    (
                        Atom("2"),
                        Struct(".", (Atom("1"), Struct(".", (Atom("3"), Atom("[]"))))),
                    ),
                ),
            ),
        )
    ...
    def test_if_then_else_pattern_works_with_cut(self):
        ...
        solutions = engine.run([Struct("if_then_else", (Atom("true_case"), r_var))])
        # With cut: only then branch
        assert len(solutions) == 1
        assert solutions[0]["R"] == Atom("then_result")

        # With "false_case", condition fails
        solutions = engine.run([Struct("if_then_else", (Atom("false_case"), r_var))])
...
class TestCutEdgeCases:
    ...
    def test_nested_cuts_respect_innermost_barrier(self):
        ...
        prog = program(
            ...
            mk_rule("inner", (Var(0, "Z"),), Struct("b", (Var(0, "Z"),))),
