...
class TestSmallPredicateOptimization:
    ...
    def test_no_performance_regression_tiny_predicates(self):
        ...
        assert (
            ms_per_k < 100.0
        ), f"Tiny predicate selection too slow: {ms_per_k:.1f}ms/1k (~{us_per_sel:.1f}µs/sel)"
    ...
    def test_semantics_identical_with_bypass(self):
        ...
        test_cases = [
            ...
            (
                ...
                Struct("small", (Int(1),)),
                2,
            ),  # Matches small(1) and small(Y)
            (
                ("small", 1),
                Struct("small", (Int(2),)),
                2,
            ),  # Type-based: matches both small(1) and small(Y)
            (("small", 1), Struct("small", (Var(11, "B"),)), 2),
            (
                ("medium", 1),
                Struct("medium", (Int(1),)),
                3,
            ),  # Type-based: all int clauses + var
            (
                ("medium", 1),
                Struct("medium", (Int(2),)),
                3,
