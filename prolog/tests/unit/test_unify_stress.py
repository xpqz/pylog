...
def test_deeply_nested_struct_unification():
    ...
    assert elapsed < time_limit(
        1.0
    ), f"Deep unification took {elapsed:.3f}s (limit: {time_limit(1.0):.1f}s)"
...
def test_many_variable_creation():
    ...
    assert elapsed < time_limit(
        1.0
    ), f"Creating {num_vars} variables took {elapsed:.3f}s"
...
def test_long_unification_chain():
    ...
    for i in range(num_vars - 1):
        result = unify(
            Var(var_ids[i]), Var(var_ids[i + 1]), store, trail, occurs_check=False
        )
...
def test_massive_trail_and_undo():
    ...
    for i in range(num_unifications):
        result = unify(
            Var(vars[i * 2]), Var(vars[i * 2 + 1]), store, trail, occurs_check=False
        )
        assert result is True
...
def test_memory_efficiency():
    ...
    vars = [
        Var(store.new_var()) if i % 10 == 0 else Int(i) for i in range(num_elements)
    ]
    ...
    assert (
        len(trail) <= expected_trail_size
    ), f"Trail too large: {len(trail)} > {expected_trail_size}"
...
def test_performance_baseline():
    ...
    if enforce_bench:
        for name, elapsed in benchmarks.items():
            assert elapsed < time_limit(
                2.0
            ), f"Benchmark {name} took {elapsed:.3f}s (> {time_limit(2.0):.1f}s)"
