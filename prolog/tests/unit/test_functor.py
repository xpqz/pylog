...
class TestFunctorExtraction:
    ...
    def test_extract_from_structure(self):
        ...
        results = engine.run(
            [
                Struct(
                    "functor",
                    (Struct("foo", (Atom("a"), Atom("b"))), Var(0, "F"), Var(1, "A")),
                )
            ]
        )
    ...
    def test_extract_from_atom(self):
        ...
        results = engine.run(
            [Struct("functor", (Atom("foo"), Var(0, "F"), Var(1, "A")))]
        )
    ...
    def test_extract_from_integer(self):
        ...
        results = engine.run([Struct("functor", (Int(42), Var(0, "F"), Var(1, "A")))])
    ...
    def test_extract_from_empty_list(self):
        ...
        # Query: functor([], F, A)
        results = engine.run(
            [Struct("functor", (Atom("[]"), Var(0, "F"), Var(1, "A")))]
        )
    ...
    def test_extract_from_list(self):
        ...
        results = engine.run([Struct("functor", (lst, Var(0, "F"), Var(1, "A")))])
    ...
    def test_extract_fails_with_unbound_first_arg(self):
        ...
        results = engine.run(
            [Struct("functor", (Var(0, "X"), Var(1, "F"), Var(2, "A")))]
        )
...
class TestFunctorConstruction:
    ...
    def test_construct_structure(self):
        ...
        results = engine.run([Struct("functor", (Var(0, "X"), Atom("foo"), Int(2)))])
        ...
        assert len({arg.id for arg in var_args}) == len(
            x.args
        ), "Args must be pairwise distinct vars"
    ...
    def test_construct_atom(self):
        ...
        results = engine.run([Struct("functor", (Var(0, "X"), Atom("foo"), Int(0)))])
    ...
    def test_construct_integer(self):
        ...
        results = engine.run([Struct("functor", (Var(0, "X"), Int(42), Int(0)))])
    ...
    def test_construct_empty_list(self):
        ...
        # Query: functor(X, [], 0)
        results = engine.run([Struct("functor", (Var(0, "X"), Atom("[]"), Int(0)))])
    ...
    def test_construct_fails_negative_arity(self):
        ...
        results = engine.run([Struct("functor", (Var(0, "X"), Atom("foo"), Int(-1)))])
    ...
    def test_construct_fails_non_atom_functor_positive_arity(self):
        ...
        results = engine.run([Struct("functor", (Var(0, "X"), Int(42), Int(2)))])
    ...
    def test_construct_with_unbound_functor(self):
        ...
        results = engine.run([Struct("functor", (Var(0, "X"), Var(1, "F"), Int(2)))])
...
class TestFunctorChecking:
    ...
    def test_check_structure_success(self):
        ...
        results = engine.run(
            [
                Struct(
                    "functor",
                    (Struct("foo", (Atom("a"), Atom("b"))), Atom("foo"), Int(2)),
                )
            ]
        )
    ...
    def test_check_structure_wrong_functor(self):
        ...
        results = engine.run(
            [
                Struct(
                    "functor",
                    (Struct("foo", (Atom("a"), Atom("b"))), Atom("bar"), Int(2)),
                )
            ]
        )
    ...
    def test_check_structure_wrong_arity(self):
        ...
        results = engine.run(
            [
                Struct(
                    "functor",
                    (Struct("foo", (Atom("a"), Atom("b"))), Atom("foo"), Int(3)),
                )
            ]
        )
    ...
    def test_check_atom_success(self):
        ...
        results = engine.run([Struct("functor", (Atom("foo"), Atom("foo"), Int(0)))])
    ...
    def test_check_integer_success(self):
        ...
        results = engine.run([Struct("functor", (Int(42), Int(42), Int(0)))])
            ...
            [
                Struct(
                    ...
                    (
                        Struct(
                            "functor",
                            (Struct("foo", (Atom("a"),)), Var(0, "F"), Var(1, "A")),
                        ),
                        Atom("fail"),
                    ),
                )
...
class TestFunctorWithVariables:
    ...
    def test_extract_unifies_functor(self):
        ...
        results = engine.run(
            [Struct("functor", (Struct("foo", (Atom("a"),)), Atom("foo"), Var(0, "A")))]
        )
    ...
    def test_extract_unifies_arity(self):
        ...
        results = engine.run(
            [
                Struct(
                    "functor",
                    (Struct("foo", (Atom("a"), Atom("b"))), Var(0, "F"), Int(2)),
                )
            ]
        )
            ...
            [
                Struct(
                    ...
                    (
                        ...
                        Struct("functor", (Var(0, "X"), Atom("foo"), Var(1, "N"))),
                    ),
                )
            ]
        )
...
class TestFunctorEdgeCases:
    ...
    def test_quoted_functor(self):
        ...
        results = engine.run(
            [Struct("functor", (Var(0, "X"), Atom("foo bar"), Int(2)))]
        )
    ...
    def test_max_arity(self):
        ...
        results = engine.run([Struct("functor", (Var(0, "X"), Atom("foo"), Int(10)))])
...
class TestFunctorWithNonIntegerArity:
    ...
    def test_non_integer_arity_fails(self):
        ...
        results = engine.run(
            [Struct("functor", (Var(0, "X"), Atom("foo"), Atom("bar")))]
        )
...
class TestFunctorAdditionalCases:
    ...
    def test_construct_dot_functor_yields_struct(self):
        ...
        # Query: functor(X, '.', 2)
        results = engine.run([Struct("functor", (Var(0, "X"), Atom("."), Int(2)))])
    ...
    def test_construct_fails_struct_functor(self):
        ...
        results = engine.run(
            [Struct("functor", (Var(0, "X"), Struct("f", (Atom("a"),)), Int(0)))]
        )
    ...
    def test_check_list_success(self):
        ...
        lst = List((Atom("a"),), Atom("[]"))
        results = engine.run([Struct("functor", (lst, Atom("."), Int(2)))])
    ...
    def test_check_empty_list_success(self):
        ...
        results = engine.run([Struct("functor", (Atom("[]"), Atom("[]"), Int(0)))])
    ...
    def test_construct_large_arity_iterative(self):
        ...
        results = engine.run([Struct("functor", (Var(0, "X"), Atom("f"), Int(100)))])
            ...
            [
                Struct(
                    ...
                    (
                        ...
                        Struct("functor", (Var(1, "X"), Var(0, "F"), Int(2))),
                    ),
                )
            ]
        )
