...
class TestArithmeticOperatorsParsing:
    ...
    def test_mixed_precedence_with_power(self):
        ...
        # (((2 ** 3) * 4) + 5)
        expected = Struct(
            "+", (Struct("*", (Struct("**", (Int(2), Int(3))), Int(4))), Int(5))
        )
        assert result == expected
    ...
    def test_arithmetic_with_is_operator(self):
        ...
        expected = Struct(
            "is", (Var(0, "X"), Struct("+", (Int(2), Struct("*", (Int(3), Int(4))))))
        )
        assert result == expected
    ...
    def test_unary_minus_with_division(self):
        ...
        reader = Reader()
        assert reader.read_term("-X / Y") == Struct(
            "/", (Struct("-", (Var(0, "X"),)), Var(1, "Y"))
        )
    ...
    def test_power_three_right_assoc(self):
        ...
        assert reader.read_term("2 ** 3 ** 2 ** 1") == Struct(
            "**", (Int(2), Struct("**", (Int(3), Struct("**", (Int(2), Int(1))))))
        )
    ...
    def test_is_with_unary_and_power(self):
        ...
        reader = Reader()
        assert reader.read_term("X is -2 ** 3") == Struct(
            "is", (Var(0, "X"), Struct("-", (Struct("**", (Int(2), Int(3))),)))
        )
        assert reader.read_term("Y is A // 3") == Struct(
            "is", (Var(0, "Y"), Struct("//", (Var(1, "A"), Int(3))))
        )
