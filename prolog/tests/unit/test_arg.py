...
class TestArgExtraction:
    ...
    def test_extract_from_unary_struct(self):
        ...
        # Query: arg(1, f(x), X)
        results = engine.run(
            [Struct("arg", (Int(1), Struct("f", (Atom("x"),)), Var(0, "X")))]
        )
    ...
    def test_extract_variable_arg(self):
        ...
        # Query: arg(1, f(Y), X)
        results = engine.run(
            [Struct("arg", (Int(1), Struct("f", (Var(1, "Y"),)), Var(0, "X")))]
        )
...
class TestArgAdditionalCases:
            ...
            [
                Struct(
                    ...
                    (
                        ...
                        Struct(
                            "arg", (Var(0, "N"), Struct("f", (Atom("a"),)), Var(1, "X"))
                        ),
                    ),
                )
            ]
    ...
    def test_arg_on_dot_struct_succeeds(self):
        ...
        # Query: arg(1, .(a,[]), X)
        results = engine.run([Struct("arg", (Int(1), pair, Var(0, "X")))])
    ...
    def test_arg_on_dot_struct_second_arg(self):
        ...
        # Query: arg(2, .(a,[]), X)
        results = engine.run([Struct("arg", (Int(2), pair, Var(0, "X")))])
    ...
    def test_arg_on_improper_dot_struct(self):
        ...
        # Query: arg(1, .(a,2), X)
        results1 = engine.run([Struct("arg", (Int(1), improper, Var(0, "X")))])

        # Query: arg(2, .(a,2), Y)
        results2 = engine.run([Struct("arg", (Int(2), improper, Var(0, "Y")))])
    ...
    def test_arg_then_other_goal_preserves_continuation(self):
        ...
        engine = Engine(program(mk_fact("q")))
        ...
        results = engine.run(
            [
                Struct(
                    ",",
                    (
                        Struct("arg", (Int(1), Struct("f", (Atom("a"),)), Var(0, "X"))),
                        Atom("q"),
                    ),
                )
            ]
...
class TestArgChecking:
    ...
    def test_check_with_unification(self):
        ...
        # Query: arg(1, f(X), a)
        results = engine.run(
            [Struct("arg", (Int(1), Struct("f", (Var(0, "X"),)), Atom("a")))]
        )
...
class TestArgBoundaryConditions:
    ...
    def test_arg_zero_fails(self):
        ...
        results = engine.run(
            [
                Struct(
                    "arg", (Int(0), Struct("foo", (Atom("a"), Atom("b"))), Var(0, "X"))
                )
            ]
        )
    ...
    def test_arg_negative_fails(self):
        ...
        results = engine.run(
            [
                Struct(
                    "arg", (Int(-1), Struct("foo", (Atom("a"), Atom("b"))), Var(0, "X"))
                )
            ]
        )
    ...
    def test_arg_on_atom_fails(self):
        ...
        # Query: arg(1, foo, X)
        results = engine.run([Struct("arg", (Int(1), Atom("foo"), Var(0, "X")))])
    ...
    def test_arg_on_integer_fails(self):
        ...
        # Query: arg(1, 42, X)
        results = engine.run([Struct("arg", (Int(1), Int(42), Var(0, "X")))])
    ...
    def test_arg_on_list_fails(self):
        ...
        results = engine.run(
            [
                Struct(
                    "arg",
                    (Int(1), List((Atom("a"), Atom("b")), Atom("[]")), Var(0, "X")),
                )
            ]
        )
...
class TestArgWithVariables:
    ...
    def test_unbound_index_fails(self):
        ...
        results = engine.run(
            [
                Struct(
                    "arg",
                    (Var(0, "N"), Struct("foo", (Atom("a"), Atom("b"))), Var(1, "X")),
                )
            ]
        )
    ...
    def test_unbound_struct_fails(self):
        ...
        # Query: arg(1, S, X)
        results = engine.run([Struct("arg", (Int(1), Var(0, "S"), Var(1, "X")))])
    ...
    def test_non_integer_index_fails(self):
        ...
        results = engine.run(
            [Struct("arg", (Atom("foo"), Struct("bar", (Atom("a"),)), Var(0, "X")))]
        )
...
class TestArgDeterminism:
    ...
    def test_deterministic_check(self):
        ...
        results = engine.run(
            [Struct("arg", (Int(1), Struct("foo", (Atom("a"), Atom("b"))), Atom("a")))]
        )
...
class TestArgEdgeCases:
    ...
    def test_deeply_nested_extraction(self):
        ...
        results = engine.run([Struct("arg", (Int(1), deep, Var(0, "X")))])
    ...
    def test_arg_with_large_arity(self):
        ...
        # Query: arg(50, big(...), X)
        results = engine.run([Struct("arg", (Int(50), big_struct, Var(0, "X")))])
        ...
        results = engine.run(
            [
                Struct(
                    ...
                    (
                        Struct(
                            "arg", (Int(1), Struct("f", (Var(0, "X"),)), Var(1, "Y"))
                        ),
                        Struct(
                            "arg",
                            (
