...
class TestIndexingEdgeCases:
    ...
    def test_all_variable_heads(self):
        ...
        expected = [{"X": Atom("a")}, {"X": Atom("b")}, {"X": Atom("c")}]
...
class TestIndexingOptimizations:
    ...
    def test_negative_integer_indexing(self):
        ...
        assert (
            len(results_neg) == 4
        ), f"Negative int query returned {len(results_neg)} results"
        assert (
            len(results_pos) == 4
        ), f"Positive int query returned {len(results_pos)} results"
        assert (
            len(results_zero) == 4
        ), f"Zero query returned {len(results_zero)} results"
