...
class TestStreamingSelection:
    ...
    def test_streaming_with_cut_early_termination(self):
        ...
        # Create many clauses
        clauses = [
            Clause(Struct("cuttest", (Int(i),)), ()) for i in range(100)  # Many clauses
        ]
    ...
    def test_generator_preserves_order(self):
        ...
        results = list(
            idx.select(("ordered", 1), Struct("ordered", (Var(2, "Z"),)), store)
        )
        ...
        results = list(
            idx.select(("ordered", 1), Struct("ordered", (Atom("first"),)), store)
        )
