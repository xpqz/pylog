...
class TestUnivDecomposition:
    ...
    def test_decompose_structure(self):
        ...
        result = engine.run(
            [
                Struct(
                    "=..",
                    (Struct("foo", (Atom("a"), Atom("b"), Atom("c"))), Var(0, "L")),
                )
            ]
        )
    ...
    def test_decompose_atom(self):
        ...
        # Query: foo =.. L
        result = engine.run([Struct("=..", (Atom("foo"), Var(0, "L")))])
    ...
    def test_decompose_atomic_integer(self):
        ...
        # Query: 42 =.. L
        result = engine.run([Struct("=..", (Int(42), Var(0, "L")))])
    ...
    def test_decompose_empty_list(self):
        ...
        # Query: [] =.. L
        result = engine.run([Struct("=..", (Atom("[]"), Var(0, "L")))])
    ...
    def test_decompose_list_structure(self):
        ...
        result = engine.run([Struct("=..", (lst_ab, Var(0, "L")))])
    ...
    def test_decompose_fails_with_unbound_left(self):
        ...
        result = engine.run([Struct("=..", (Var(0, "X"), Var(1, "L")))])
...
class TestUnivConstruction:
    ...
    def test_construct_structure(self):
        ...
        result = engine.run([Struct("=..", (Var(0, "X"), lst))])
...
class TestUnivBidirectional:
    ...
    def test_check_mode(self):
        ...
        lst = List((Atom("foo"), Atom("a"), Atom("b")), Atom("[]"))
        result = engine.run(
            [Struct("=..", (Struct("foo", (Atom("a"), Atom("b"))), lst))]
        )
        ...
        result = engine.run(
            [
                Struct(
                    ",",
                    (
                        Struct("=..", (Struct("foo", (Atom("a"),)), Var(0, "L"))),
                        Atom("fail"),
                    ),
                )
            ]
...
class TestUnivTrailing:
    ...
    def test_trailing_on_backtrack(self):
        ...
        prog = program(mk_fact("p", Atom("a")), mk_fact("p", Atom("b")))
        engine = Engine(prog)
            ...
            [
                Struct(
                    ...
                    (
                        ...
                        Struct(
                            "=..",
                            (Var(0, "X"), List((Atom("foo"), Var(1, "Y")), Atom("[]"))),
                        ),
                    ),
                )
            ]
    ...
    def test_trailing_undo_on_fail_then_redo(self):
        ...
        prog = program(mk_fact("q", Atom("a")), mk_fact("q", Atom("b")))
        engine = Engine(prog)
...
class TestUnivEdgeCases:
    ...
    def test_decompose_list_nested_dot_shape(self):
        ...
        # Query: [a,b] =.. L
        result = engine.run([Struct("=..", (lst, Var(0, "L")))])
