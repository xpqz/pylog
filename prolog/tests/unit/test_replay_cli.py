...
class TestReplayToolCLI:
    ...
    def test_replay_cli_output_format(self, tmp_path, capsys):
        ...
        # Test JSON output
        with patch.object(sys, 'argv', ['replay_trace', str(trace_file), '--format', 'json']):
            replay_main()
...
class TestAnalyzeToolCLI:
    ...
    def test_analyze_cli_basic_usage(self, tmp_path, capsys):
        ...
        captured = capsys.readouterr()
        assert "Total Events: 100" in captured.out or "100 events" in captured.out.lower()
    ...
    def test_analyze_cli_json_output(self, tmp_path, capsys):
        ...
        with patch.object(sys, 'argv', ['analyze_trace', str(trace_file), '--format', 'json']):
            analyze_main()
    ...
    def test_analyze_cli_hot_predicates(self, tmp_path, capsys):
        ...
        with patch.object(sys, 'argv', ['analyze_trace', str(trace_file), '--top', '3']):
            analyze_main()
    ...
    def test_analyze_cli_output_file(self, tmp_path, capsys):
        ...
        with patch.object(sys, 'argv', ['analyze_trace', str(trace_file), '-o', str(output_file)]):
            analyze_main()
    ...
    def test_analyze_cli_backtrack_analysis(self, tmp_path, capsys):
        ...
        with patch.object(sys, 'argv', ['analyze_trace', str(trace_file), '--backtrack']):
            analyze_main()
