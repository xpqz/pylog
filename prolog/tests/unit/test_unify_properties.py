...
def random_struct(
    store: Store, var_pool: List[int], depth: int, rng: random.Random
) -> Struct:
    ...
    if depth > 3:  # Limit nesting
        ...
        args = tuple(
            random_term(store, var_pool, depth + 1, rng) for _ in range(num_args)
        )
...
def random_list(
    ...
    if depth > 3:  # Limit nesting
        ...
        items = tuple(
            random_term(store, var_pool, depth + 1, rng) for _ in range(num_items)
        )
...
def reify(term: Any, store: Store) -> Any:
    ...
    if isinstance(term, Var):
        ...
        return PrologList(
            tuple(reify(i, store) for i in term.items), tail=reify(term.tail, store)
        )
    else:
...
def terms_structurally_equal(t1: Any, t2: Any) -> bool:
    ...
    if isinstance(t1, Atom):
        ...
        return (
            ...
            and all(
                terms_structurally_equal(a1, a2) for a1, a2 in zip(t1.args, t2.args)
            )
        )
    elif isinstance(t1, PrologList):
        return (
            len(t1.items) == len(t2.items)
            and all(
                terms_structurally_equal(i1, i2) for i1, i2 in zip(t1.items, t2.items)
            )
            and terms_structurally_equal(t1.tail, t2.tail)
        )
    else:
        return False
...
def test_unify_symmetry_property(seed):
    ...
    assert (
        result1 == result2
    ), f"Symmetry violated: unify(t1,t2)={result1}, unify(t2,t1)={result2}"
...
def test_unify_idempotence_property(seed):
    ...
    if result1:
        ...
        assert (
            len(trail) == mark2
        ), f"Second unification added {len(trail) - mark2} trail entries"
...
def test_trail_invertibility_property(seed):
    ...
    assert (
        restored_snapshot == initial_snapshot
    ), "Store not fully restored after undoing all unifications"
    assert (
        len(trail) == initial_trail_len
    ...
    assert (
        final_snapshot2 == final_snapshot
...
def test_reified_terms_equal_on_success(seed):
    ...
    if result:
        ...
        def normalize_list(term):
            ...
            if (
                isinstance(term, PrologList)
                and len(term.items) == 0
                and term.tail != Atom("[]")
            ):
        ...
        assert terms_structurally_equal(
            r1_norm, r2_norm
