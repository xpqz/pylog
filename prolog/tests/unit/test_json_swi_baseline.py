"""
SWI-Prolog baseline tests for JSON functionality.

This module tests JSON support against SWI-Prolog behavior to ensure
compatibility with the SWI-Prolog JSON library.

Tests cover:
- json_read/3 and json_write/3 (classic mode)
- json_read_dict/3 and json_write_dict/3 (dict mode)
- atom_json_term/3 (bidirectional conversion)
- JSON constant handling (null, true, false)
- Complex nested structures
- Error handling for malformed JSON
"""

import pytest
from prolog.engine.engine import Engine
from prolog.parser.reader import Reader
from prolog.tests.helpers import program


@pytest.mark.swi_baseline
class TestJSONSWIBaseline:
    """Test JSON support against SWI-Prolog behavior."""

    def test_basic_json_parsing_classic_mode(self, swi):
        """Test basic JSON parsing matches SWI-Prolog (classic mode)."""
        # Test simple object in classic mode
        test_program = """
        :- use_module(library(http/json)).

        test_json_parse :-
            atom_json_term('{"name": "test", "value": 42}', Term, []),
            Term = json([name-test, value-42]).
        """

        # Test in PyLog
        reader = Reader()
        engine = Engine(program())
        query = reader.read_term(
            'atom_json_term(\'{"name": "test", "value": 42}\', Term, [])'
        )
        result_pylog = list(engine.solve(query))

        # Test in SWI-Prolog if available
        count_swi = swi.count(test_program, "test_json_parse")

        # Both should succeed
        assert len(result_pylog) == 1  # PyLog should find solution
        assert count_swi == 1  # SWI should find solution

    def test_basic_json_parsing_dict_mode(self, swi):
        """Test basic JSON parsing in dict mode."""
        # Test simple object in dict mode
        test_program = """
        :- use_module(library(http/json)).

        test_json_dict :-
            atom_json_term('{"name": "test", "value": 42}', Term, [json_object(dict)]),
            get_dict(name, Term, test),
            get_dict(value, Term, 42).
        """

        # Test in PyLog
        reader = Reader()
        engine = Engine(program())
        query = reader.read_term(
            'atom_json_term(\'{"name": "test", "value": 42}\', Term, [json_object(dict)])'
        )
        result_pylog = list(engine.solve(query))

        # Test in SWI-Prolog
        count_swi = swi.count(test_program, "test_json_dict")

        # Both should succeed
        assert len(result_pylog) == 1
        assert count_swi == 1

    def test_json_constants_classic_mode(self, swi):
        """Test JSON constant handling in classic mode (null, true, false)."""
        # Test JSON constants
        test_program = """
        test_json_constants :-
            atom_json_term('{"null_val": null, "bool_true": true, "bool_false": false}', Term, []),
            Term = json([null_val-@(null), bool_true-@(true), bool_false-@(false)]).
        """

        # Test in PyLog
        reader = Reader()
        engine = Engine(program())
        query = reader.read_term(
            'atom_json_term(\'{"null_val": null, "bool_true": true, "bool_false": false}\', Term, [])'
        )
        result_pylog = list(engine.solve(query))

        # Test in SWI-Prolog
        count_swi = swi.count(test_program, "test_json_constants")

        # Both should handle constants correctly
        assert len(result_pylog) == 1
        assert count_swi == 1

    def test_json_arrays_classic_mode(self, swi):
        """Test JSON array parsing in classic mode."""
        test_program = """
        test_json_array :-
            atom_json_term('[1, 2, "hello", true]', Term, []),
            Term = [1, 2, hello, @(true)].
        """

        # Test in PyLog
        reader = Reader()
        engine = Engine(program())
        query = reader.read_term("atom_json_term('[1, 2, \"hello\", true]', Term, [])")
        result_pylog = list(engine.solve(query))

        # Test in SWI-Prolog
        count_swi = swi.count(test_program, "test_json_array")

        assert len(result_pylog) == 1
        assert count_swi == 1

    def test_json_empty_structures(self, swi):
        """Test empty JSON objects and arrays."""
        test_program = """
        test_empty :-
            atom_json_term('{}', json([]), []),
            atom_json_term('[]', [], []).
        """

        # Test in PyLog
        reader = Reader()
        engine = Engine(program())
        query1 = reader.read_term("atom_json_term('{}', json([]), [])")
        result1 = list(engine.solve(query1))

        query2 = reader.read_term("atom_json_term('[]', [], [])")
        result2 = list(engine.solve(query2))

        # Test in SWI-Prolog
        count_swi = swi.count(test_program, "test_empty")

        assert len(result1) == 1
        assert len(result2) == 1
        assert count_swi == 1

    def test_json_number_types(self, swi):
        """Test JSON number handling (integers and floats)."""
        test_program = """
        test_numbers :-
            atom_json_term('{"int": 42, "float": 3.14, "zero": 0}', Term, []),
            Term = json([int-42, float-3.14, zero-0]).
        """

        # Test in PyLog
        reader = Reader()
        engine = Engine(program())
        query = reader.read_term(
            'atom_json_term(\'{"int": 42, "float": 3.14, "zero": 0}\', Term, [])'
        )
        result_pylog = list(engine.solve(query))

        # Test in SWI-Prolog
        count_swi = swi.count(test_program, "test_numbers")

        assert len(result_pylog) == 1
        assert count_swi == 1

    def test_json_error_handling_malformed(self, swi):
        """Test error handling for malformed JSON."""
        # Test malformed JSON should fail in both systems
        test_program = """
        test_malformed :-
            atom_json_term('{"bad": json}', _, []).
        """

        # Test in PyLog
        reader = Reader()
        engine = Engine(program())
        query = reader.read_term("atom_json_term('{\"bad\": json}', _, [])")
        result_pylog = list(engine.solve(query))

        # Test in SWI-Prolog
        count_swi = swi.count(test_program, "test_malformed")

        # Both should fail (no solutions)
        assert len(result_pylog) == 0
        assert count_swi == 0

    def test_atom_json_term_bidirectional(self, swi):
        """Test bidirectional conversion with atom_json_term/3."""
        test_program = """
        :- use_module(library(http/json)).

        test_bidirectional :-
            Term = json([name-test, value-42]),
            atom_json_term(Atom, Term, []),
            atom_json_term(Atom, Term2, []),
            Term = Term2.
        """

        # Test in PyLog - test just the basic conversion first
        reader = Reader()
        engine = Engine(program())
        query = reader.read_term(
            "Term = json([name-test, value-42]), atom_json_term(Atom, Term, [])"
        )
        result_pylog = list(engine.solve(query))

        # Test in SWI-Prolog
        count_swi = swi.count(test_program, "test_bidirectional")

        assert len(result_pylog) == 1
        assert count_swi == 1
