...
class TestBasicExecution:
    ...
    def test_trivial_fact_query(self):
        ...
        prog = program(mk_fact("bird", Atom("robin")))
        engine = Engine(prog)
    ...
    def test_conjunction_associativity(self):
        ...
        sols2 = engine.run(
            [Struct(",", (Struct(",", (Atom("a"), Atom("b"))), Atom("c")))]
        )
    ...
    def test_builtin_then_call_left_to_right(self):
        ...
        solutions = engine.run(
            [
                Struct(
                    ",",
                    (Struct("=", (Var(0, "X"), Int(1))), Struct("q", (Var(0, "X"),))),
                )
            ]
        )
        assert len(solutions) == 1
    ...
    def test_rule_body_execution(self):
        ...
        prog = program(
            ...
            mk_rule("flies", (Var(0, "X"),), Struct("bird", (Var(0, "X"),))),
        )
        engine = Engine(prog)
    ...
    def test_no_recursion_in_control_flow(self):
        ...
        for i in range(depth - 1):
            clauses.append(mk_rule(f"p{i}", (), Atom(f"p{i+1}")))
    ...
    def test_same_functor_different_arity(self):
        ...
        prog = program(mk_fact("p", Int(1)), mk_fact("p", Int(1), Int(2)))
        engine = Engine(prog)
...
class TestEngineInvariants:
    ...
    def test_heights_restored_exactly(self):
        ...
        solutions = engine.run(
            [Struct(",", (Struct("p", (Var(0, "X"),)), Struct("q", (Var(1, "Y"),))))]
        )
    ...
    def test_ports_order(self):
        ...
        prog = program(mk_fact("p", Int(1)), mk_fact("p", Int(2)))
        engine = Engine(prog)
    ...
    def test_pop_frame_executes_once(self):
        ...
        prog = program(mk_rule("q", (), Atom("true")))
        engine = Engine(prog)
...
class TestBuiltinsTopLevel:
    ...
    def test_builtin_with_control(self):
        ...
        solutions = engine.run(
            [Struct(";", (Struct(",", (Atom("true"), Atom("true"))), Atom("fail")))]
        )
...
class TestBacktracking:
    ...
    def test_conjunction_backtracking(self):
        ...
        # Query: a(X), b(Y).
        solutions = engine.run(
            [Struct(",", (Struct("a", (Var(0, "X"),)), Struct("b", (Var(1, "Y"),))))]
        )
        assert len(solutions) == 4
    ...
    def test_disjunction(self):
        ...
        prog = program(mk_fact("a"), mk_fact("b"))
        engine = Engine(prog)

        # Query: (a ; b).
        solutions = engine.run([Struct(";", (Atom("a"), Atom("b")))])
        assert len(solutions) == 2
        ...
        solutions = engine.run(
            [
                Struct(
                    ",",
                    (
                        Struct(";", (Struct("=", (Var(0, "X"), Int(1))), Atom("true"))),
...
class TestControlConstructs:
    ...
    def test_cut_basic(self):
        ...
        prog = program(
            ...
            mk_fact("a", Int(2)),
            mk_rule(
                "test", (Var(0, "X"),), Struct("a", (Var(0, "X"),)), Struct("!", ())
    ...
    def test_cut_does_not_escape_callee(self):
        ...
        prog = program(
            # q :- (p ; r), s.
            mk_rule("q", (), Struct(";", (Atom("p"), Atom("r"))), Atom("s")),
            # p :- !, fail.
    ...
    def test_if_then_else_true_condition(self):
        ...
        prog = program(mk_fact("cond"), mk_fact("then_branch"), mk_fact("else_branch"))
        engine = Engine(prog)
    ...
    def test_if_then_else_false_condition(self):
        ...
        prog = program(mk_fact("then_branch"), mk_fact("else_branch"))
        engine = Engine(prog)
...
class TestIfThenElse:
    ...
    def test_then_commits_after_first_cond_success(self):
        ...
        prog = program(mk_fact("a", Int(1)), mk_fact("a", Int(2)))
        engine = Engine(prog)
        ...
        solutions = engine.run(
            [
                Struct(
                    ...
                    (
                        Struct(
                            "->", (Atom("fail"), Struct("=", (Var(0, "X"), Int(1))))
                        ),
                        Struct("=", (Var(0, "X"), Int(2))),
                    ),
                )
        ...
        prog = program(
            ...
            mk_rule(
                ...
                Struct(
                    ";",
                    (
                        Struct("->", (Struct("a", (Var(0, "X"),)), Struct("!", ()))),
                        Atom("true"),
                    ),
                ),
                Struct("=", (Var(0, "X"), Int(1))),
    ...
    def test_then_nondet_enumerates_after_commit(self):
        ...
        prog = program(
            mk_fact("cond"), mk_fact("y", Atom("b")), mk_fact("y", Atom("c"))
        )
        engine = Engine(prog)
    ...
    def test_else_nondet_enumerates_when_cond_fails(self):
        ...
        prog = program(mk_fact("y", Int(1)), mk_fact("y", Int(2)))
        engine = Engine(prog)
    ...
    def test_control_cp_restores_heights(self):
        ...
        # Query: (a -> true ; true).
        solutions = engine.run(
            [Struct(";", (Struct("->", (Atom("a"), Atom("true"))), Atom("true")))]
        )
        assert len(solutions) == 1
...
class TestVariableIsolation:
    ...
    def test_fresh_vars_per_clause_instance(self):
        ...
        prog = program(
            mk_rule("t", (Var(0, "X"),), Struct("=", (Var(0, "X"), Int(1)))),
            mk_rule("t", (Var(0, "X"),), Struct("=", (Var(0, "X"), Int(2)))),
        )
        engine = Engine(prog)
        ...
        solutions = engine.run(
            [Struct(",", (Struct("t", (Var(0, "X"),)), Struct("t", (Var(1, "Y"),))))]
        )
        assert len(solutions) == 4
        expected = [
            (Int(1), Int(1)),
...
class TestFrameAndChoicepoint:
    ...
    def test_bind_then_fail_restores(self):
        ...
        prog = program(
            mk_rule(
                "p", (Var(0, "X"),), Struct("=", (Var(0, "X"), Int(1))), Atom("fail")
            ),
            mk_rule("p", (Var(0, "X"),), Struct("=", (Var(0, "X"), Int(2)))),
        )
        engine = Engine(prog)
    ...
    def test_restore_heights_for_predicate_cp(self):
        ...
        solutions = engine.run(
            [Struct(",", (Struct("p", (Var(0, "X"),)), Struct("q", (Var(0, "X"),))))]
        )
        assert len(solutions) == 2
    ...
    def test_deep_backtracking(self):
        ...
        prog = program(
            mk_rule("p", (Var(0, "X"),), Struct("q", (Var(0, "X"),))),
            mk_rule("q", (Var(0, "X"),), Struct("r", (Var(0, "X"),))),
            mk_rule("r", (Var(0, "X"),), Struct("s", (Var(0, "X"),))),
            mk_fact("s", Int(1)),
            mk_fact("s", Int(2)),
        )
        engine = Engine(prog)
...
class TestSolutionLimits:
    ...
    def test_max_solutions_simple(self):
        ...
        prog = program(mk_fact("a", Int(1)), mk_fact("a", Int(2)), mk_fact("a", Int(3)))
    ...
    def test_max_solutions_with_cut(self):
        ...
        prog = program(
            ...
            mk_fact("a", Int(2)),
            mk_fact("a", Int(3)),
            mk_rule("p", (Var(0, "X"),), Struct("a", (Var(0, "X"),)), Struct("!", ())),
        )
...
class TestErrorHandling:
    ...
    def test_occurs_check(self):
        ...
        prog = program(mk_fact("unify", Var(0, "X"), Struct("f", (Var(0, "X"),))))
        engine = Engine(prog, occurs_check=True)

        # Query: unify(Y, Y).
        # Should fail due to occurs check (Y = f(Y) is infinite)
        solutions = engine.run([Struct("unify", (Var(0, "Y"), Var(0, "Y")))])
        assert len(solutions) == 0
    ...
    def test_occurs_check_in_clause(self):
        ...
        prog = program(mk_fact("cycle", Var(0, "X"), Struct("g", (Var(0, "X"),))))
        engine = Engine(prog, occurs_check=True)

        # Query: cycle(Y, Y).
        # Should fail (Y = g(Y) is cyclic)
        solutions = engine.run([Struct("cycle", (Var(0, "Y"), Var(0, "Y")))])
        assert len(solutions) == 0
    ...
    def test_record_solution_variable_order(self):
        ...
        solutions = engine.run(
            [
                Struct(
                    "append",
                    (PrologList((Int(1), Int(2))), PrologList((Int(3),)), Var(0, "Z")),
                )
            ]
        )
        assert len(solutions) == 1
...
class TestDeepPrograms:
    ...
    def test_deep_chain_no_recursion(self):
        ...
        for i in range(depth - 1):
            clauses.append(
                mk_rule(f"c{i}", (Int(i),), Struct(f"c{i+1}", (Int(i + 1),)))
            )
...
class TestNondetBuiltins:
    ...
    def test_flip_builtin_scaffold(self):
        ...
        prog = program(mk_fact("flip", Int(0)), mk_fact("flip", Int(1)))
        engine = Engine(prog)
        ...
        solutions = engine.run(
            [
                Struct(
                    ";",
                    (
                        Struct(",", (Struct("flip", (Var(0, "X"),)), Struct("!", ()))),
                        Struct("=", (Var(0, "X"), Int(2))),
                    ),
                )
            ]
...
class TestIntegration:
    ...
    def test_list_member(self):
        ...
        prog = program(
            # member(X, [X|_]).
            mk_fact("member", Var(0, "X"), PrologList((Var(0, "X"),), Var(1, "_"))),
    ...
    def test_append(self):
        ...
        prog = program(
            # append([], L, L).
            mk_fact("append", PrologList(()), Var(0, "L"), Var(0, "L")),
        ...
        solutions = engine.run([Struct("append", (l1, l2, Var(0, "X")))])
        assert len(solutions) == 1
    ...
    def test_factorial(self):
        ...
        prog = program(
            ...
            mk_rule(
                ...
                Struct("is", (Var(1, "F"), Struct("*", (Var(0, "N"), Var(3, "F1"))))),
            ),
        )
        engine = Engine(prog)
