...
class TestFindallFreeVariables:
    ...
    def test_findall_with_compound_goal_using_disjunction(self):
        """findall/3 should handle disjunction in compound goals with outer variables."""
        program_text = """
        parent(john, mary).
        parent(john, tom).
        parent(mary, ann).
        ...
        # P = john, findall(C, (parent(P, C), (male(C) ; female(C))), Children)
        # P is bound in outer context, find all children of john (male or female)
        query = Struct(
            ",",
            (
                Struct("=", (Var(0, "P"), Atom("john"))),
        ...
        children = solutions[0]["Children"]
        assert isinstance(children, List)
        # Should find mary and tom (both children of john)
        assert len(children.items) == 2
        found_children = set(children.items)
        assert found_children == {Atom("mary"), Atom("tom")}
...
@pytest.mark.iso
class TestBagofSetofFreeVariables:
    """Test bagof/3 and setof/3 handling of variables from outer context."""

    def test_bagof_with_outer_bound_variable_simple(self):
        """bagof/3 should use outer binding for variables in goal but not template."""
        program_text = """
        color(red).
        color(blue).
        color(green).
        ...
        # X = red, bagof(Y, has_color(Y, X), Things)
        # X is bound in outer context, should find all things with color red
        query = Struct(
            ",",
            (
    ...
    def test_setof_with_outer_bound_variable_simple(self):
        """setof/3 should use outer binding and return sorted unique results."""
        program_text = """
        has_num(a, 3).
        has_num(b, 1).
        has_num(c, 2).
        ...
        # findall to get all items first, then use setof with outer variable
        # X = 1, setof(Y, has_num(Y, X), Things)
        # X is bound in outer context, should find all things with num 1
        query = Struct(
            ",",
            (
                ...
                Struct(
                    ...
                    (
                        Var(1, "Y"),
                        Struct("has_num", (Var(1, "Y"), Var(0, "X"))),
                        Var(2, "Things"),
                    ),
                ),
            ),
        ...
        things = solutions[0]["Things"]
        assert isinstance(things, List)
        # Should find b and d (both have num 1), sorted
        assert len(things.items) == 2
        # Check ordering: b comes before d in atom ordering
        assert things.items[0] == Atom("b")
        assert things.items[1] == Atom("d")

    def test_bagof_with_compound_goal_using_outer_var(self):
        """bagof/3 should handle compound goals with outer variables."""
        program_text = """
        parent(john, mary).
        parent(john, tom).
        parent(mary, ann).
        ...
        # P = john, bagof(C, (parent(P, C), male(C)), Sons)
        # P is bound in outer context, find all male children of john
        query = Struct(
            ",",
            (
    ...
    @pytest.mark.swi_baseline
    def test_bagof_free_vars_swi_baseline(self, swi):
        """Compare bagof free variable behavior with SWI-Prolog."""
        program = """
        has_color(apple, red).
        has_color(sky, blue).
        has_color(fire, red).
        ...
        # Test with outer bound variable - X is bound in outer context
        goal = "X = red, bagof(Y, has_color(Y, X), Things)"
        count = swi.count(program, goal)
        # SWI should succeed with one solution
        assert count == 1

        # Extract the Things list to verify it contains apple and fire
        goal2 = "X = red, bagof(Y, has_color(Y, X), Things), member(Item, Things)"
        items = swi.onevar(program, goal2, "Item")
        # Should find apple and fire (both have color red)
        assert len(items) == 2
        assert "apple" in items
    ...
    @pytest.mark.swi_baseline
    def test_setof_free_vars_swi_baseline(self, swi):
        """Compare setof free variable behavior with SWI-Prolog."""
        program = """
        has_num(a, 1).
        has_num(b, 1).
        has_num(c, 2).
        ...
        # Test with outer bound variable - X is bound in outer context
        goal = "X = 1, setof(Y, has_num(Y, X), Things)"
        count = swi.count(program, goal)
        # SWI should succeed with one solution
        assert count == 1

        # Extract the Things list to verify ordering and content
        goal2 = "X = 1, setof(Y, has_num(Y, X), Things), member(Item, Things)"
        items = swi.onevar(program, goal2, "Item")
        # Should find a and b (both have num 1), sorted
        assert len(items) == 2
        assert "a" in items
        assert "b" in items
