"""Unit tests for the WAM unify instruction family.

These cover unify_variable and unify_value in both modes. Write mode is
reached two ways and both are exercised: put_structure, which always builds,
and get_structure against an unbound variable, which falls back to building.

Ported from the set_variable/set_value tests added with the Phase 1 unify/set
instruction families. Those two instructions were dropped rather than carried
forward: their implementations were the write-mode branches of unify_variable
and unify_value line for line, and no code generator emits them.
"""
...
class TestUnifyVariableWriteModeViaPutStructure:
    """unify_variable in write mode, entered through put_structure."""

    def test_unify_variable_creates_new_ref(self):
        """unify_variable Xi in write mode creates a new REF and writes it to the heap."""
        m = Machine()
        m.code = [
            (OP_PUT_STRUCTURE, ("f", 1), 0),  # mode=write, S=2
            (OP_UNIFY_VARIABLE, 0),  # Create REF, write to heap, assign to X0
        ]
    ...
    def test_unify_variable_multiple_calls_in_write_mode(self):
        """Successive unify_variable instructions create distinct REFs."""
        m = Machine()
        m.code = [
            (OP_PUT_STRUCTURE, ("f", 2), 0),
            (OP_UNIFY_VARIABLE, 0),  # X0
            (OP_UNIFY_VARIABLE, 1),  # X1
        ]
    ...
    def test_unify_variable_extends_x_registers_in_write_mode(self):
        """unify_variable extends the X register bank as needed in write mode."""
        m = Machine()
        m.code = [
...
class TestUnifyValueWriteModeViaPutStructure:
    """unify_value in write mode, entered through put_structure."""

    def test_unify_value_writes_register_to_heap(self):
        """unify_value Xi in write mode writes Xi's address to the heap."""
        m = Machine()
        ...
        m.code = [
            ...
            (OP_UNIFY_VALUE, 0),  # Write X0 to the heap
        ]
    ...
    def test_unify_value_writes_constant(self):
        """unify_value can write constant addresses."""
        m = Machine()
    ...
    def test_unify_value_writes_structure(self):
        """unify_value can write structure addresses."""
        m = Machine()
        ...
        m.code = [
            ...
            (OP_UNIFY_VALUE, 0),  # Write X0 (g structure) as arg
        ]
    ...
    def test_unify_value_sequential_writes(self):
        """Successive unify_value instructions write multiple values."""
        m = Machine()
        ref1 = new_ref(m)
        ref2 = new_ref(m)
        m.X = [ref1, ref2]
        ...
        m.code = [
            ...
            (OP_UNIFY_VALUE, 0),  # Write X0 as first arg
            (OP_UNIFY_VALUE, 1),  # Write X1 as second arg
        ]
...
class TestUnifyVariableReadMode:
    """unify_variable in read mode."""
    ...
    def test_unify_variable_reads_from_heap(self):
        ...
        m.code = [
            (OP_PUT_STRUCTURE, ("f", 1), 1),  # Build f/_ in A1
            (OP_UNIFY_VALUE, 0),  # Write const as arg
            # Now read it back
    ...
    def test_unify_variable_multiple_reads(self):
        ...
        m.code = [
            ...
            (OP_UNIFY_VALUE, 0),  # Write arg1
            (OP_UNIFY_VALUE, 1),  # Write arg2
            # Now read it
...
class TestUnifyValueReadMode:
    """unify_value in read mode."""
...
class TestUnifyVariableWriteModeViaGetStructure:
    """unify_variable in write mode, entered through get_structure."""
...
class TestUnifyValueWriteModeViaGetStructure:
    """unify_value in write mode, entered through get_structure."""
...
class TestStructureBuilding:
    """Building terms on the heap in write mode."""

    def test_build_simple_structure(self):
        """Build f(X) using put_structure + unify_variable."""
        m = Machine()
        m.code = [
            (OP_PUT_STRUCTURE, ("f", 1), 1),  # Build in A1, not A0
            (OP_UNIFY_VARIABLE, 0),  # X0 gets the new REF
        ]
    ...
    def test_build_structure_with_constant(self):
        """Build f(42) using put_structure + unify_value."""
        m = Machine()
        const_addr = new_con(m, 42)
        m.X = [const_addr]
    ...
    def test_build_nested_structure(self):
        ...
        m.code = [
            ...
            (OP_UNIFY_VARIABLE, 0),  # X in X0
            # Build f(g(X), Y)
            (OP_PUT_STRUCTURE, ("f", 2), 3),  # Build in A3
            (OP_UNIFY_VALUE, 2),  # Write g(X) from A2
            (OP_UNIFY_VARIABLE, 1),  # Y in X1
        ]
...
class TestStructureMatching:
    """Matching terms already on the heap in read mode."""
    ...
    def test_match_nested_structure(self):
        ...
        m.code = [
            ...
            (OP_UNIFY_VALUE, 0),  # Write const
            # Build f(g(42))
            (OP_PUT_STRUCTURE, ("f", 1), 2),  # Build f/_ in A2
            (OP_UNIFY_VALUE, 1),  # Write g structure
            # Now match it
...
class TestMixedSequences:
    ...
    def test_build_then_match(self):
        ...
        m.code = [
            ...
            (OP_UNIFY_VARIABLE, 0),  # X0 = new REF
            # Match against it
...
class TestEdgeCases:
    ...
    def test_unify_variable_zero_arity(self):
        """unify_variable against a zero-arity structure (shouldn't happen, but handle it)."""
        m = Machine()
        m.code = [
    ...
    def test_unify_value_with_none_writes_none(self):
        """unify_value with None in the register writes None."""
        m = Machine()
        m.X = [None]
...
class TestMachineState:
    ...
    def test_write_mode_instructions_increment_p(self):
...
class TestSRegisterManagement:
    ...
    def test_s_advances_with_unify_variable_in_write_mode(self):
        """S advances after unify_variable in write mode."""
        m = Machine()
        m.code = [
